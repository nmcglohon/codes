...
static void dragonfly_read_config(const char * anno, dragonfly_param *params)
{
    ...
    netMan = DragonflyNetworkManager(p->total_routers, p->total_terminals, p->num_routers, p->intra_grp_radix, p->num_global_channels, p->num_cn, 1, 1, max_hops_per_group, max_global_hops_nonminimal);
    ...
    while (fread(&newInterLink, sizeof(InterGroupLink), 1, systemFile) != 0) {
            ...
            int mapped_src_gid = read_src_id_global;
            int mapped_dest_gid = read_dest_id_global;
...
void terminal_dally_init( terminal_state * s, tw_lp * lp )
{
   ...
   int num_lps = codes_mapping_get_lp_count(lp_group_name, 1, LP_CONFIG_NM_TERM,
    ...
    if(s->terminal_id == 0)
    {
        ...
        fprintf(rdfdally_file, "time destination source qosclass num.hops latency");
#endif
			...
			tw_lp * lp )
{
    ...
    int dest_router_id = dfdally_get_assigned_router_id_from_terminal(s->params, msg->dfdally_dest_terminal_id, 0);
    // int dest_router_id = codes_mapping_get_lp_relative_id(msg->dest_terminal_lpid, 0, 0) / s->params->num_cn;
