    ...
    switch (conn.conn_type)
    {
        ...
        case CONN_GLOBAL:
            ...
            if(_used_inter_ports < _max_inter_ports) {
                ...
                fprintf(stderr, " ===== At location 12-- sgid:%d sprt:%d sgrid:%d dgid:%d dgrid:%d size:%d _ gcsizeA: %d || ", conn.src_gid, conn.port, conn.src_group_id, conn.dest_gid, conn.dest_group_id, globalConnections[conn.dest_gid].size(), globalConnections.size());
                globalConnections[conn.dest_gid].push_back(conn);
                fprintf(stderr, "%d _ %d\n", globalConnections[conn.dest_gid].size(), globalConnections.size());
                _used_inter_ports++;
                _connected_to_router_gids.insert(conn.dest_gid);
            }
            else
